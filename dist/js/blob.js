/*
 * this blob.js file is a customized fork from a demo on codrops:
			...
			const onMouseMoveFn = (ev) => {
				requestAnimationFrame(() => {
					if ( !this.started ) {
						...
						const scaleX = mousepos.x < this.win.width/4 ? lineEq(tilt.sx[0],tilt.sx[1],this.win.width/4,0,mousepos.x) : lineEq(tilt.sx[1],tilt.sx[0],this.win.width,this.win.width/4,mousepos.x);
						const scaleY = mousepos.y < this.win.height/4 ? lineEq(tilt.sy[0],tilt.sy[1],this.win.height/4,0,mousepos.y) : lineEq(tilt.sy[1],tilt.sy[0],this.win.height,this.win.height/4,mousepos.y);
